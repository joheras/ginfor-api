...
from model.indexer import GoogleNetIndexer
...
for (i, paths) in enumerate(dataset.chunk(imagePaths, conf["googlenet_batch_size"])):
	# load the set of images from disk and describe them
	(labels, images) = dataset.build_batch(paths, conf["googlenet_fixed_size"])
	controlImages = dataset.build_batch_control(paths, conf["googlenet_fixed_size"])
	...
	if i > 0:
		oi._debug("processed {} images".format((i + 1) * conf["googlenet_batch_size"],
			msgType="[PROGRESS]"))
